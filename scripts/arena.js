...
const env = { ...filteredEnv, ...(tierOverride ? { TIER_OVERRIDE: tierOverride } : {}), SKIP_COUNT: skipCount, ...(nameOverride ? { CHAR_NAME: nameOverride } : {}) };
...
for (let i = 0; i < N; i++) {
  try {
    execSync('npx wdio --spec android/specs/arena.e2e.js', { stdio, env });
